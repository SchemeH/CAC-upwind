...
%                   the Heston problem.
%  比较不同求解器在Heston问题上的cpu时间与误差  
% DESCRIPTION:
%   Plots error against cpu time in the discretized Heston PDE.在离散化的Heston PDE中绘制误差与cpu时间的关系  
%   We only include the ode15s, phipm and Crank--Nicolson methods.我们只包括ode15s, phipm和克兰克-尼科尔森方法 
%   For implementation details of the ADI methods the reader is
%   referred to the paper by In't Hout in the references.
%   关于ADI方法的实现细节，读者可以参考参考资料中In't Hout的论文
% Parameters  参数
K = 100;
Ls = 800;
Lv = 5;
...
% applied to multi-dimensional diffusion equations with mixed 应用于含混合导数项的多维扩散方程
...
% Set up the correct meshgrid for ploting  为绘图设置正确的网格网格
I = find(s<=Smax); 
s = s(I);
Nps = length(s);
I = find(v<=Vmax);
...
for i = 1:length(Tol)
  ...
  fprintf('*** PHIPM ***\n');
  tic;
  SolExp = phipm(tspan(end), A, [U0, b], Tol(i), false);
  tm = toc;
...
clf;
axes('FontSize', 12);
loglog(cpuExp-(1:0.25:2.75), ErrExp, 'ko-', 'LineWidth', 2);
hold on
