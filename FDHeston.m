...
%
% PARAMETERS:
%   ds     - length between grid points in the s domain.s域中网格点之间的长度。
%   dv     - length between grid points in the v domain.v域中网格点之间的长度。
%   Ns     - number of grid points in the s domain.s域中的网格点数
%   Nv     - number of grid points in the v domain.
%   rd     - domestic interest rate.
%   rf     - foreign interest rate.
%   nu     - mean reversion level.均值回归的水平
%   kappa  - rate at which we revert to the mean value.我们恢复到平均值的速率
%   lambda - volatility factor波动率的因素
%   rho    - correlation between underlying Brownian motions.基本布朗运动之间的相关性
%
% RETURNS:
...
% The vector b with boundary effects 带有边界效应的向量b
