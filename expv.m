...
%  general matrix A using Krylov subspace  projection techniques.一般矩阵A的Krylov子空间投影技术  
%  It does not compute the matrix exponential in isolation but instead,它不是孤立地计算矩阵指数，而是，
%  it computes directly the action of the exponential operator on the 它直接计算指数算符对操向量
%  operand vector. This way of doing so allows for addressing large这种方法允许处理大型的稀疏问题  
%  sparse problems. The matrix under consideration interacts only 所考虑的矩阵仅通过矩阵向量乘积(无矩阵方法)相互作用。 
...
%
%  [w, err] = expv( t, A, v )
%  renders an estimate of the error on the approximation.在近似上给出误差的估计
%
%  [w, err] = expv( t, A, v, tol )
%  overrides default tolerance.覆盖默认的公差。
%
%  [w, err, hump] = expv( t, A, v, tol, m )
%  overrides default tolerance and dimension of the Krylov subspace,重写Krylov子空间的默认公差和维数，
%  and renders an approximation of the `hump'.并给出一个近似的“驼峰”。
%
%  The hump is defined as:
%          hump = max||exp(sA)||, s in [0,t]  (or s in [t,0] if t < 0).
%  It is used as a measure of the conditioning of the matrix exponential它被用来衡量矩阵指数的条件作用  
%的问题  
%  problem. The matrix exponential is well-conditioned if hump = 1,矩阵指数是良条件的
...
%  Example 2:
%  ----------
%    % generate a random sparse matrix 生成一个随机稀疏矩阵
%    n = 100;
%    A = rand(n);
%    for j = 1:n
%        for i = 1:n
